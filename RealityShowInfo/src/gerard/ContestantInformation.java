...
//		province = Character.toUpperCase(province.charAt(0)) + province.substring(1).toLowerCase();
	 ...
	 */
	public void setStNum(int stNum)throws  InvalidInputException {
		for(int f = 0; f <= Integer.toString(stNum).length(); f++ ){
			if (!Character.isDigit(Integer.toString(stNum).charAt(f)))
	...
	public void setPostalCode(String postalCode) throws InvalidInputException{
		...
		if(postalCode.length() > 6)
			...
			throw new InvalidInputException("Please make the fourth character a nember.");
		else if (!Character.isDigit(postalCode.charAt(4)))
